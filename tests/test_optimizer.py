...
from mypo.optimizer import MinimumVarianceBaseOptimizer, SharpRatioBaseOptimizer
...
def test_minimum_variance_optimizer() -> None:
    ...
    optimizer = MinimumVarianceBaseOptimizer()
...
def test_minimum_variance_optimizer_with_minimum_return() -> None:
    ...
    optimizer = MinimumVarianceBaseOptimizer(minimum_return=0.10)
    optimizer.optimize(market)
    weights = optimizer.get_weights()
...
def test_minimum_sharp_ratio_optimizer() -> None:
    market = Market.load(TEST_DATA)
    market = market.extract(market.get_index()[:200])
    optimizer = SharpRatioBaseOptimizer(risk_free_rate=0.02)
    optimizer.optimize(market)
    weights = optimizer.get_weights()
...
def test_semi_minimum_variance_optimizer() -> None:
    market = Market.load(TEST_DATA)
    market = market.extract(market.get_index()[:200])
    optimizer = MinimumVarianceBaseOptimizer(with_semi_covariance=True)
    optimizer.optimize(market)
    weights = optimizer.get_weights()
